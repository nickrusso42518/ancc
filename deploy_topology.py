...
def main(base_url, snapshot_name):
    ...
    with open(f"{in_dir}/p2p_only.json", "r", encoding="utf-8") as handle:
    ...
    with httpx.Client() as client:
        ...
        for i, (node, attr) in enumerate(topology["nodes"].items()):
            ...
            tmpl_id = OS_TATTR_MAP[attr["Configuration_Format"]]["tmpl_id"]
            ...
            # TODO different per device type?
            # Upload the startup configs from the Batfish snapshot
            with open(f"snapshots/pre/configs/{node.upper()}.txt", "r", encoding="utf-8") as handle:
            ...
            # Retain deployment response to add links later
            node_dict[node] = depl
